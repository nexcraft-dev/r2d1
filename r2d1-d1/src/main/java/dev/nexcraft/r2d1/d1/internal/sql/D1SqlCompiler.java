...
import dev.nexcraft.r2d1.d1.internal.metadata.D1IndexedField;
import dev.nexcraft.r2d1.d1.internal.metadata.D1Metadata;
import dev.nexcraft.r2d1.d1.internal.metadata.D1ValueType;
...
public final class D1SqlCompiler {

  public D1Statement clear(D1CollectionMetadata metadata) {
    Objects.requireNonNull(metadata, "metadata");
    return D1Statement.of("DELETE FROM " + D1Metadata.quoteIdentifier(metadata.collection()));
  }

  public D1Statement upsert(D1CollectionMetadata metadata, IndexEntry entry) {
    Objects.requireNonNull(metadata, "metadata");
    ...
    for (D1IndexedField field : metadata.indexedFields()) {
      ...
      placeholders.add(
          addParameter(parameters, parameter(field, entry.values().get(field.name()))));
    }
  ...
  public D1Statement delete(D1CollectionMetadata metadata, DocumentKey key) {
    Objects.requireNonNull(metadata, "metadata");
  ...
  public CompiledQuery query(D1CollectionMetadata metadata, IndexQuery query) {
    Objects.requireNonNull(metadata, "metadata");
    ...
    for (IndexQuery.Filter filter : query.filters()) {
      ...
      D1Parameter parameter = parameter(field, filter.value());
  ...
  public IndexPage page(D1CollectionMetadata metadata, CompiledQuery query, D1Result result) {
    Objects.requireNonNull(metadata, "metadata");
    Objects.requireNonNull(query, "query");
  ...
  private static String cursorCondition(
    ...
    String sortParameter = addParameter(parameters, parameter(sortField, sortValue));
    String idParameter =
        addParameter(parameters, new D1Parameter.TextParameter(cursor.lastDocumentKey().id()));
  ...
  private static D1Parameter parameter(D1IndexedField field, IndexValue value) {
    field.type().requireCompatible(field.name(), value);
